	...
	return {
		...
		init: function () {
			...
			this.mobileScreenBreakpoint =
				MiluParams.mobileScreenBreakpoint;
			this.tabletScreenBreakpoint =
				MiluParams.tabletScreenBreakpoint;
			this.notebookScreenBreakpoint =
				MiluParams.notebookScreenBreakpoint;
			this.desktopScreenBreakpoint =
				MiluParams.desktopScreenBreakpoint;
			this.desktopBigScreenBreakpoint =
				MiluParams.desktopBigScreenBreakpoint;
		...
		setClasses: function () {
			...
			if (
				...
				!MiluParams.forceAnimationMobile
			) {
		...
		stickyMenu: function (scrollTop) {
			...
			scrollPoint =
				parseInt(MiluParams.stickyMenuScrollPoint, 10) ||
				$('body').offset().top;
			...
			if (menuOffset) {
				if ('100%' === MiluParams.menuOffset) {
					scrollPoint =
						$(window).height() -
						parseInt(MiluParams.desktopMenuHeight, 10);
				} else {
					scrollPoint =
						menuOffset -
						parseInt(MiluParams.desktopMenuHeight, 10);
		...
		parallax: function () {
			var smallScreen =
				...
				MiluParams.parallaxNoSmallScreen;
		...
		toggleMenu: function () {
			$(document).on('click', '.toggle-mobile-menu', function (event) {
				...
				$(window).trigger('milu_mobile_menu_toggle_button_click', [
					$(this),
				]);
			...
			$(document).on('click', '.toggle-side-panel', function (event) {
				event.preventDefault();
				$(window).trigger('milu_side_panel_toggle_button_click', [
					$(this),
				]);
			...
			$(document).on('click', '.toggle-offcanvas-menu', function (event) {
				...
				$(
					window
				).trigger('milu_offcanvas_menu_toggle_button_click', [
					$(this),
				]);
			...
			$(document).on('click', '.toggle-overlay-menu', function (event) {
				...
				$(window).trigger(
					'milu_overlay_menu_toggle_button_click',
					[$(this)]
				);
			});
			...
			$(document).on('click', '.toggle-lateral-menu', function (event) {
				event.preventDefault();
				$(window).trigger(
					'milu_lateral_menu_toggle_button_click',
					[$(this)]
				);
		...
		scrollToMainContent: function () {
			...
			$('html, body')
				...
				.animate(
					...
					parseInt(MiluParams.smoothScrollSpeed, 10),
					MiluParams.smoothScrollEase,
					function () {
						if ('' !== hash) {
		...
		wowAnimate: function () {
			...
			if (doWow && 'undefined' !== typeof WOW) {
				wowAnimate = new WOW({
					offset: MiluParams.WOWAnimationOffset,
				...
				searchInput.on('keyup', function (event) {
					...
					if (8 === event.keyCode || 46 === event.keyCode) {
						...
						timer = setTimeout(function () {
							...
							$.post(MiluParams.ajaxUrl, data, function (
								response
							) {
								if ('' !== response) {
		...
		setInternalLinkClass: function () {
			...
			$.each(MiluParams.allowedMimeTypes, function (
				index,
				value
			) {
				regEx += '|' + value;
			...
			if (MiluParams.isWooCommerce) {
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						MiluParams.WooCommerceAccountUrl
					)
				) {
					$(
						'a[href^="' +
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						MiluParams.WooCommerceCheckoutUrl
					)
				) {
					$(
						'a[href^="' +
		...
		videoThumbnailPlayOnHover: function () {
			...
			$('iframe.youtube-bg', itemsContainer).each(function () {
				if (
					'undefined' !== typeof MiluYTVideoBg &&
					MiluYTVideoBg.players[
				...
				if ($iframe.length) {
					if ($iframe.hasClass('youtube-bg')) {
						$(this).on('mouseenter', function () {
							if (
								'undefined' !== typeof MiluYTVideoBg &&
								MiluYTVideoBg.players[YTPlayerId]
							) {
								MiluYTVideoBg.players[
									YTPlayerId
								].playVideo();
